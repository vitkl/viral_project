...
filename = paste0("./processed_data_files/predict_domain_human_clust",gsub("-","","2018-08-19"),".RData")
```
...
permuteCount = function(data, select_nodes = NULL, also_permuteYZ = F, N_permut = 10, clustermq_jobs = 60, clustermq_mem = 12500){
    permutationPval(interactions2permute = IDs_interactor_human_A ~ IDs_interactor_human_B, # first set of interacting pairs (XY) that are to be permuted
                    ...
                    clustermq = T, clustermq_mem = clustermq_mem,
                    split_comp_inner_N = 4, clustermq_jobs = clustermq_jobs,
...
if(!"res_count_all" %in% objects()){
    ...
    res_count_all = permuteCount(data_all, select_nodes = IDs_domain_human_B ~ domain_count >= 1,
                                 N_permut = 5000, clustermq_jobs = 100, clustermq_mem = 12500)
    proc.time() - time
    # save results
...
random_domains = function(N = 100, seed = seed, Ns = seq(25, 1000, 40), res){
    set.seed(seed)
