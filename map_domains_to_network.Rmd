...
clusterExport(cl, c("InterProScan_domains_list"), envir=environment())

InterProScan_domains_list = parSapply(cl, InterProScan_domains_list, function(InterProScan_domain){
    Nseq = length(InterProScan_domain)
    reduced = reduce(InterProScan_domain, with.revmap=T)
    metadata_orig = mcols(InterProScan_domain)
    ...
    for (i in 1:length(reduced$revmap)) {
        number = reduced$revmap[[i]]
        mapping[number] = i
    }
    metadata_new_list = split(metadata_new, mapping)
    new_metadata_new_list = SplitDataFrameList()
    for (i in 1:length(metadata_new_list)) {
        temp_metadata = metadata_new_list[[i]]
        ...
        new_metadata_new_list[[i]] = unique(temp_metadata)
    }
...
InterProScan_domains_list = as(InterProScan_domains_list,"GRangesList")
InterProScan_domains_nonred = unlist(InterProScan_domains_list, use.names=F)
```
