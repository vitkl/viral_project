...
plotVenn = function(res, ...){
    ...
    venn.plot <- VennDiagram::draw.pairwise.venn(
        ...
        category = c("відкриті екземпляри", "відомі екземпляри"),
...
plotAnnotVenn = function(res, main = "", sub = "",
                         ...
                         cex = 1.8, cat.cex = 1.8,
                         fontfamily = "Times New Roman", ...) { # "serif" "Arial" "Times New Roman"
...
mPlotAnnotVenn = function(filter_by_domain_data = NULL,
                          ...
                          motif_pval_tags = c("низький поріг", "оптимальний поріг", "суворий поріг"),
        ...
        annotated_venns = lapply(motif_pval_cutoffs, function(motif_pval_cutoff){
            ...
            annotated_venn = plotAnnotVenn(res_instances,
                                           main = paste0(motif_pval_tags[motif_pval_cutoffs == motif_pval_cutoff],
                                                         ": значення p < ", signif(motif_pval_cutoff, 2)),
                                           sub = paste0(res_instances$N_query_prot_with_known_instances,
                                                        " білки з відомими мотивами (",
                                                        res_instances$N_query_prot_with_known_instances_found," відкрито заново)\n",
                                                        res_proteins$N_query_total_instances_found," мотиви передбачені в інших білках"),
...
figureVenn = function(filter_by_domain_data = NULL,
    ...
    IntAct_grob = textGrob("взаємодії білків людини\n(база даних IntAct) ",
    ...
    Vidal_grob = textGrob("взаємодії білків людини\n(дані групи Vidal)",
                           gp=gpar(fontsize=25, fontfamily = fontfamily,
                                   lineheight = 1.4, ...))
    ################################################################################ viral only
    res_list_viral = myBenchmarkMotifs(neg_set = "all_instances",
    ...
    viral_grob = textGrob("взаємодії між білками \nлюдини та вірусів",
                          gp=gpar(fontsize=25, fontfamily = fontfamily,
                                  lineheight = 1.4, ...))
...
#####################################################################################
replotFigureVenn = function(figure, main, fontfamily = "Times New Roman", ...){
    IntAct_grob = textGrob("взаємодії білків людини\n(база даних IntAct) ",
...
figureMODLIGDOC2 = replotFigureVenn(figureMODLIGDOC,
                 main = "")
ggsave(filename = "ukr_Venn_not_filtered_MOD_LIG_DOC.svg", plot = figureMODLIGDOC2$big_plot_text,
...
figureMODLIGDOC2_filtered2 = replotFigureVenn(figureMODLIGDOC2_filtered,
                 main = "")
ggsave(filename = "ukr_Venn_filtered_MOD_LIG_DOC.svg", plot = figureMODLIGDOC2_filtered2$big_plot_text,
       device = "svg", width = 19.3, height = 11.60638, units = "in")
#grid.newpage()
...
figureMODLIGDOC2DEGCLV2 = replotFigureVenn(figureMODLIGDOC2DEGCLV,
                 main = "")
ggsave(filename = "ukr_Venn_not_filtered_MOD_LIG_DOC_DEG_CLV.svg", plot = figureMODLIGDOC2DEGCLV2$big_plot_text,
       device = "svg", width = 19.3, height = 11.60638, units = "in")
#grid.newpage()
#grid.draw(figure2$big_plot_text)
...
figureMODLIGDOC2DEGCLV_filtered2 = replotFigureVenn(figureMODLIGDOC2DEGCLV_filtered,
                 main = "")
ggsave(filename = "ukr_Venn_filtered_MOD_LIG_DOC_DEG_CLV.svg", plot = figureMODLIGDOC2DEGCLV_filtered2$big_plot_text,
       device = "svg", width = 19.3, height = 11.60638, units = "in")
#grid.newpage()
#grid.draw(figure_filtered2$big_plot_text)
...
figureMODLIGDOC2DEGCLVTRG2 = replotFigureVenn(figureMODLIGDOC2DEGCLVTRG,
                 main = "")
ggsave(filename = "ukr_Venn_not_filtered_MOD_LIG_DOC_DEG_CLV_TRG.svg", plot = figureMODLIGDOC2DEGCLVTRG2$big_plot_text,
       device = "svg", width = 19.3, height = 11.60638, units = "in")
#grid.newpage()
#grid.draw(figure2$big_plot_text)
...
figureMODLIGDOC2DEGCLVTRG_filtered2 = replotFigureVenn(figureMODLIGDOC2DEGCLVTRG_filtered,
                 main = "")
ggsave(filename = "ukr_Venn_filtered_MOD_LIG_DOC_DEG_CLV_TRG.svg", plot = figureMODLIGDOC2DEGCLVTRG_filtered2$big_plot_text,
       device = "svg", width = 19.3, height = 11.60638, units = "in")
#grid.newpage()
#grid.draw(figure_filtered2$big_plot_text)
#####################################################################################
...
dom[IDs_domain_human %in% domains_known_mapped, SLIM_binding := "так"]
dom[!IDs_domain_human %in% domains_known_mapped, SLIM_binding := "ні / не відомо"]
...
setnames(dom, "SLIM_binding", "зв'язують мотиви")
family_std = "Times New Roman"
ggplot(dom, aes(p.value, color = `зв'язують мотиви`, fill = `зв'язують мотиви`)) +
    ...
    theme(panel.grid.major =  element_line(color = 'grey', size = 0.2, linetype = 'solid'),
          ...
          legend.text = element_text(family=family_std),
          axis.title = element_text(family=family_std),
          axis.text.x = element_text(family=family_std),
          axis.text.y = element_text(family=family_std)) +
    xlab("Значення p-value") + ylab("Щільність розподілу") # 10.34 * 12.38 cm = 4.0708661 * 4.8740157 in

ks.test(dom[SLIM_binding == "ні / не відомо", p.value],
        dom[SLIM_binding == "так", p.value], 
        alternative = "less")
...
```{r more_motif_summaries, eval=FALSE}
...
```{r write_cytoscape_files, eval=FALSE}
IntAct_net = bechmark2Net(occurence_query = IntAct$res$qslimfinder.Full_IntAct3.FALSE$occurence_query,
                        matching_known = IntAct$res$qslimfinder.Full_IntAct3.FALSE$overlapping_GRanges_query,
                        top_domain = F)
IntAct_net = readableNet(IntAct_net)
...
session_info.
filename = paste0("./processed_data_files/compr_benchmarking_ukr_ukr_Venn_IntVidVir",format(Sys.Date., "%Y%m"),"_type_", measure1, measure2,".RData")
