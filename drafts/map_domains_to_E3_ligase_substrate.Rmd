---
title: "Mapping domains to the E3 ubiquitin ligase - substrates protein interaction network"
...
I read interaction data and clean this data to make it more useble. Then, I filter all human interactions for only interactions of E3 ligases and any other protein (but not E3 ligase). I identify E3 ligases by the protein name (uniprot query: name:e3+name:ligase+organism:9606, + means AND)
...
E3ligases_url = "http://www.uniprot.org/uniprot/?query=name:e3+name:ligase+organism:9606&format=tab&columns=id"
E3ligases = fread(E3ligases_url, stringsAsFactors = F)$Entry
E3ligasesinteractions = subsetMITABbyID(all_human_interaction, ID_seed = E3ligases, within_seed = F, only_seed2nonseed = T)
E3ligases_within_interactions = subsetMITABbyID(all_human_interaction, ID_seed = fread(E3ligases_url, stringsAsFactors = F)$Entry, within_seed = T, only_seed2nonseed = F)
```

E3 ligase proteins have `r length(unique(E3ligasesinteractions$data$pair_id))` known interacting pairs with human proteins while only `r length(unique(E3ligases_within_interactions$data$pair_id))` with other viral proteins.  
