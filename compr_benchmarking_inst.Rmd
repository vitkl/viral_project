...
```{r, viral}
res_list_viral = mBenchmarkMotifs(datasets = c("Full_IntAct", "Vidal", "Mann"), 
            ...
            minoverlap = 2, maxgap = 0, motif_setup_month = "201710",
...
```{r, viral_w_domains}
res_list_viral_w_domains = mBenchmarkMotifs(datasets = c("Full_IntAct", "Vidal", "Mann"), 
            descriptions = c("human network (full IntAct) searched \nfor motifs present in viral proteins",
                             "human network (Vidal's data only) searched \nfor motifs present in viral proteins",
                             "human network (Mann's data only) searched \nfor motifs present in viral proteins"),
            dir = "./", 
...
```{r, kinase, eval=FALSE}
res_list_kinases = mBenchmarkMotifs(datasets = c("Full_IntAct", "Vidal", "Mann", "withinVidal", "withinMann"), #c("", "Vidal", "Mann", "withinVidal", "withinMann")
            ...
            dir = "../kinase_project/",
            merge_domain_data = T, neg_set = neg_set,
            domain_res_file = "../kinase_project/processed_data_files/what_we_find_VS_ELM_clust20171019.RData",
            domain_results_obj = "res_count", motif_input_obj = "forSLIMFinder_Ready",
            one_from_cloud = T, type = "QSLIMFinder",
...
mBenchmarkMotifsROC(res_list_kinases, data_type = "both", col_query = colors[1], col_all = colors[2], lwd = 4, text_args = "col = \"black\" | cex = 2", legend_args = "x = 0.5 | y = 0.4 | y.intersp = 1.5 | box.col = \"white\"", measure1 = measure1, measure2 = measure2)
```
...
```{r, znf, eval=FALSE}
res_list_znf = mBenchmarkMotifs(datasets = c("Full_IntAct", "Vidal", "Mann", "withinVidal", "withinMann"), 
            ...
            merge_domain_data = T, neg_set = neg_set,
            domain_res_file = "../zinkfinger_project/processed_data_files/what_we_find_VS_ELM_clust20171019.RData",
            domain_results_obj = "res_count", motif_input_obj = "forSLIMFinder_Ready",
            one_from_cloud = T, type = "QSLIMFinder",
            ...
            seed = 21, N = N_negative_sets, replace = T, within1sequence = T,
            query_predictor_col = query_predictor_col, all_predictor_col = "Sig", normalise = T,
            minoverlap = 2, maxgap = 0, motif_setup_month = "201711",
            count_ranges_by = list(by = "IDs_domain_human", name = "motif_occ_per_domain",
                                   normalise_by = "domain_count", normalised_name = "normalised_motif_occ_per_domain"),
            filter_by_domain_data = filter_by_domain_data,
            dir_suff = "")

mBenchmarkMotifsROC(res_list_znf, data_type = "both", col_query = colors[1], col_all = colors[2], lwd = 4, text_args = "col = \"black\" | cex = 2", legend_args = "x = 0.5 | y = 0.4 | y.intersp = 1.5 | box.col = \"white\"", measure1 = measure1, measure2 = measure2)
```
...
filter_by_domain_data_print = gsub("<*","",filter_by_domain_data_print)
filter_by_domain_data_print = gsub("=*","",filter_by_domain_data_print)
filename = paste0("./processed_data_files/compr_benchmarking_inst",format(Sys.Date(), "%Y%m"),"_",paste0(motif_types,collapse = "_"),"_",neg_set,"_",query_predictor_col,"_", filter_by_domain_data_print,"_type_", measure1, measure2,".RData")
