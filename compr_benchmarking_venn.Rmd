...
figureVenn = function(filter_by_domain_data = NULL,
    ...
    if(is.null(motif_pval_cutoffs_BioPlex)){
        motif_pval_cutoffs_BioPlex = c(lenient_cutoff,
                                       1 - min(rocr_list_BioPlex[1,1][[1]]@cutoffs[[1]][rocr_list_BioPlex[2,1][[1]]@y.values[[1]] >=
                                                                                            rocr_list_BioPlex[2,1][[1]]@x.values[[1]]], na.rm = T),
                                       1 - min(rocr_list_BioPlex[1,1][[1]]@cutoffs[[1]][rocr_list_BioPlex[2,1][[1]]@y.values[[1]] >= stringent_prec],
                                               na.rm = T))
...
# Filter by human-human and viral-human domain predictions - (4 human have the same top-1 domain)
figureMODLIGDOC_4 = figureVenn(filter_by_domain_data = "p.value < 0.5",
                               main = "Filter by human-human and viral-human domain predictions (2 human protein have the same top-1 domain)",
                               motif_pval_tags = c("lenient cutoff", "optimal cutoff", "stringent cutoff"),
                               datasets = datasets,
                               non_query_domain_results_obj = "res_count_all", # res_count_all
                               query_domains_only = F, min_non_query_domain_support = 0, min_top_domain_support4motif_nq = 2,
                               return_all = F, merge_domain_data = T, descriptions = descriptions,
                               viral_only_venn = T)
...
```{r protein_domain_motif_protein_network}
...
IntAct_filt_hum_dom = myBenchmarkMotifs(neg_set = "all_proteins",
                                   filter_by_domain_data = "p.value < 0.5",
                                   motif_pval_cutoff = figureMODLIGDOC_4$motif_pval_cutoffs_IntAct[2], # figureMODLIGDOC_4$motif_pval_cutoffs_IntAct,
                                   dataset = datasets[1], datasets = datasets,
                                   return_all = T, 
                                   non_query_domain_results_obj = "res_count_all", # res_count_all
                                   query_domains_only = F, min_non_query_domain_support = 0,
                                   min_top_domain_support4motif_nq = 4, merge_domain_data = T)
...
instances = instances[!is.na(bench_res_cyt$occurence_query$IDs_domain_human)]
...
IntAct_net = bechmark2Net(occurence_query = IntAct$res$qslimfinder.Full_IntAct3.FALSE$occurence_query,
                        matching_known = IntAct$res$qslimfinder.Full_IntAct3.FALSE$overlapping_GRanges_query)
IntAct_filt_hum_dom_net = bechmark2Net(occurence_query = IntAct_filt_hum_dom$res$qslimfinder.Full_IntAct3.FALSE$occurence_query,
                        matching_known = IntAct_filt_hum_dom$res$qslimfinder.Full_IntAct3.FALSE$overlapping_GRanges_query,
...
```{r ELM_domains, fig.height=6, fig.width=7}
...
```{r slim_domain_ranks, fig.height=6, fig.width=7}
...
```{r slim_interactions, fig.height=6, fig.width=7}
...
dom3[, N_pairs := uniqueN(paste0(query, domain)), by = .(network, Correct_SLiM_binding)]
...
```{r domain_confusion matrix, eval=FALSE}
...
# use 
bench_res_cyt = IntAct_filt_hum_dom_net$res$qslimfinder.Full_IntAct3.FALSE
IntAct_net = bechmark2Net(occurence_query = bench_res_cyt$occurence_query,
                          matching_known = bench_res_cyt$overlapping_GRanges_query,
                          top_domain_by = c(NA))
IntAct_net = readableNet(IntAct_net)
