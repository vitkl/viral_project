...
packages = c("MItools", "RColorBrewer", "devtools", "grid","gridExtra", "ggplot2", "VennDiagram", "svglite", "GenomicRanges", "Biostrings")
...
descriptions = c("human network (full IntAct) searched \nfor motifs present in viral proteins",
                 "human network (BioPlex data, cloudfixF) searched \nfor motifs present in viral proteins",
...
figureVenn = function(filter_by_domain_data = NULL,
    ...
    big_plot = gridExtra::arrangeGrob(big_plot_IntAct, big_plot_BioPlex, big_plot_viral, nrow=1)
    big_text = gridExtra::arrangeGrob(IntAct_grob, BioPlex_grob, viral_grob, nrow=1)
...
IntAct_optimal = myBenchmarkMotifs(neg_set = "all_proteins",
                  filter_by_domain_data = "p.value < 0.5",
                  motif_pval_cutoff = 0.039,
                  dataset = datasets[1], datasets,
                  return_all = T, merge_domain_data = T)
...
dom_human = R.utils::env(load("./processed_data_files/predict_domain_human_clust20180819.RData"))
dom_human = dom_human$res_count_all$data_with_pval
...
dom = merge(dom, elm_interactions,
            by.x = c("protein_with_motif", "protein_with_domain", "domain"),
            by.y = c("interactorElm", "interactorDomain", "InterProID"),
            all.x = T, all.y = F)
dom[!is.na(Elm), SLIM_interaction := "yes"]
dom[is.na(Elm), SLIM_interaction := "no / not known"]
...
This is a more direct benchmark but will be pretty unfair because most of these interactions are not described  

```{r slim_interactions, fig.height=6, fig.width=7}
# include protein_with_domain to benchmark domain identification in individual interactions or exclude to bechmark the ability to identify correct recognition for a given protein domain
dom3 = unique(dom[,.(protein_with_motif, domain, p.value, domain_type, SLIM_interaction, network)])
ggplot(dom3, aes(p.value, color = SLIM_interaction, fill = SLIM_interaction)) +
