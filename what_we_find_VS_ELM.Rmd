...
test_enrichment = function(N, res, domains_known_mapped, random = F, seed = 1, name = ""){
    if(random) {set.seed(seed); domains_found = res$data_pval[sample(1:nrow(res$data_pval), N), unique(IDs_domain_human)]}
    if(!random){
    ...
    return(c(test$p.value, test$estimate, table_res["TRUE", "TRUE"], name = name))
}

enrichment = sapply(seq(25, 500, 25), test_enrichment, res, domains_known_mapped)
colnames(enrichment) = seq(25, 500, 25)
enrichment_low_back = sapply(seq(25, 500, 25), test_enrichment, res_low_back, domains_known_mapped)
colnames(enrichment_low_back) = seq(25, 500, 25)
enrichment_low_deg = sapply(seq(25, 500, 25), test_enrichment, res_low_deg, domains_known_mapped)
colnames(enrichment_low_deg) = seq(25, 500, 25)
enrichment_low_deg_back = sapply(seq(25, 500, 25), test_enrichment, res_low_deg_back, domains_known_mapped)
...
enrichmentRANDOM = sapply(seq(25, 500, 25), test_enrichment, res, domains_known_mapped, random = T, seed = 1)
...
plotEnrichment = function(..., domains_known_mapped, type = "count", plot_type = plot_type){
    ...
    leg_pos_y = max(sapply(res, function(x, typenum) max(x[typenum,]), typenum))
    leg_pos_x = max(sapply(res, function(x) max(as.numeric(colnames(x))))) * 0.16
    if(type == "count") plot(colnames(res[[1]]), resres[[1]][typenum,], ylab = "known domain found", xlab = "top N viral protein - domain pairs selected", col = color[1], type = plot_type, ylim = c(0,length(domains_known_mapped)+1))
    if(type != "count") plot(colnames(resres[[1]]), resres[[1]][typenum,], ylab = "known domain found", xlab = "top N viral protein - domain pairs selected", col = color[1], type = plot_type, ylim = c(0,leg_pos_y+1))
    for (i in 2:ngroups) {
        lines(x = colnames(res[[i]]), y = res[[i]][typenum,], col = color[2], type = plot_type)
    }
    if(type == "count") abline(h = length(domains_known_mapped), col = color[ngroups + 1])
    legend(x = leg_pos_x, y = leg_pos_y, 
           c("statictic used in permutation test:",
             sapply(res, function(x) x)
             "domain frequency among interactors of a viral protein", "Fisher test pval: domain overrepresentation over the background", "N random proteins", "domains known to interact with linear motifs"), 
...
lines(x = colnames(enrichment), y = enrichmentRANDOM[2,], col = "violet", type = "l")
legend(x = 80, y = max(enrichment[2,]) - 1, c("statictic used in permutation test:","domain frequency among interactors of a viral protein", "Fisher test pval: domain overrepresentation over the background", "N random proteins"), col = c("white","red", "blue", "violet"), lty = 1 ,merge = TRUE)
```
