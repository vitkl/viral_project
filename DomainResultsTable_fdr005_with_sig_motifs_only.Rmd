...
if(!file.exists(name)){
    ...
    entry.list = getInterProEntryTypes("./entry.list")
    rm(list = ls()[!ls() %in% c("printTable","XYZ.p.adjust","res_count", "sequential_filter", "name", "occurence_QSLIMFinder", "comparimotif_wdb", "patterns_QSLIMFinder", "entry.list")])
...
X = printMotifDomainTable(res_count, doman_viral_pairs = doman_viral_pairs, motifs = motifs, destfile = "./domains_fdr_corrected_only_with_significant_motifs_empirical_p_value.tsv",
               ...
               patterns_QSLIMFinder = patterns_QSLIMFinder, print_table = F, entry.list = entry.list)
```
